...
import kotlinx.android.synthetic.main.view_search_old.view.searchClear
import kotlinx.android.synthetic.main.view_search_old.view.searchContent

class SearchViewOld @JvmOverloads constructor(
    context: Context,
    ...
    init {
        View.inflate(context, R.layout.view_search_old, this)
    ...
    private fun applyAttrs(attributeSet: AttributeSet) = context.useAttributes(attributeSet, R.styleable.SearchView) {
        val hint = it.getString(R.styleable.SearchViewOld_android_hint)
        setHint(hint)

        val icon = it.getResourceIdOrNull(R.styleable.SearchViewOld_icon)
        setIcon(icon)
    }
}
